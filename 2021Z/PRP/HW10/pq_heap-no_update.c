...
//asi bych nekopíroval celý heap a nepushoval ho od začátku - radši využít funkce pq_down, pq_swap ?
...
_Bool pq_update(void *_pq, int label, int cost){
   ...
   if (pq && pq->len < pq->size && label >= 0 && label < pq->size && pq->heapIDX[label] != -1 ) {
       int current = pq->heapIDX[label];
       pq->cost[current] = cost;
       int parent = GET_PARENT(current);
       // swap current and parent until heap is satisfied
       while (current >= 1 && pq->cost[parent] > pq->cost[current]) {
           pq_swap(pq, parent, current);
           current = parent;
